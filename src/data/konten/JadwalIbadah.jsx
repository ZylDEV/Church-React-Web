...
// Komponen untuk menampilkan daftar jadwal
const JadwalListSection = ({ title, icon: Icon, jadwalItems }) => {
      ...
      >
        {jadwalItems.map((item) => (
          <motion.div
            ...
            className="group bg-white/80 backdrop-blur-xl rounded-3xl shadow-lg border border-gray-200/50 p-6 hover:shadow-2xl hover:bg-white/90 transition-all duration-300 transform hover:-translate-y-1 cursor-pointer"
            ...
            <div className="flex flex-col lg:flex-row lg:items-center gap-4 lg:gap-6">
              ...
              <div className="flex items-start gap-4 flex-1">
                <div className="w-14 h-14 lg:w-16 lg:h-16 bg-gradient-to-br from-indigo-100 via-purple-100 to-blue-100 rounded-2xl flex items-center justify-center flex-shrink-0 group-hover:from-indigo-200 group-hover:via-purple-200 group-hover:to-blue-200 transition-colors shadow-sm">
                  <Bell className="w-7 h-7 lg:w-8 lg:h-8 text-indigo-600" />
                </div>
                ...
                <div className="flex-1 min-w-0">
                  <h4 className="text-lg lg:text-xl font-bold text-gray-900 mb-2 group-hover:text-indigo-700 transition-colors leading-tight">
                    {item.jadwal || "Ibadah Tanpa Judul"}
                  </h4>
              ...
              {expandedItems[item.id] && (
                ...
                >
                  <div className="flex flex-col gap-2 text-gray-700">
                    {item.pelayananFirman && (
                      <div className="flex items-center gap-2">
                        <Speaker size={18} className="text-gray-600" />
                        <span className="text-sm">
                          <span className="font-semibold">Pelayan Firman:</span>{" "}
                          {item.pelayananFirman}
                        </span>
                      </div>
                    )}

                    {item.pembacaanAktivitas && (
                      <div className="flex items-center gap-2">
                        <BookOpen size={18} className="text-gray-600" />
                        <span className="text-sm">
                          <span className="font-semibold">Pembacaan Alkitab:</span>{" "}
                          {item.pembacaanAktivitas}
                        </span>
                      </div>
                    )}

                    {/* Lokasi hanya untuk Gabungan */}
                    {item.tipe === "Gabungan" && item.lokasi && (
                      <div className="flex items-center gap-2">
      ...
      (snapshot) => {
        ...
        if (val) {
          const data = Object.entries(val).map(([id, item]) => ({
            id,
            ...item,
            tipe: item.jadwal?.toLowerCase().includes("mingguan")
              ? "Mingguan"
              : "Gabungan",
          }));
  ...
  const filteredJadwalList = useMemo(() => {
    ...
    if (lowerCaseSearchTerm) {
      filtered = filtered.filter((item) =>
        item.jadwal?.toLowerCase().includes(lowerCaseSearchTerm) ||
        item.pelayananFirman?.toLowerCase().includes(lowerCaseSearchTerm)
      );
    }

    if (selectedMonth) {
      filtered = filtered.filter((item) => item.tanggal?.startsWith(selectedMonth));
    }

    return filtered;
  }, [jadwalList, searchTerm, selectedMonth]);

  const jadwalMingguan = filteredJadwalList.filter(
    (item) => item.tipe === "Mingguan"
  );
  const jadwalGabungan = filteredJadwalList.filter(
    (item) => item.tipe === "Gabungan"
  );
  const totalJadwal = filteredJadwalList.length;
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 lg:w-12 lg:h-12 bg-gradient-to-br from-indigo-500 via-purple-500 to-blue-600 rounded-2xl flex items-center justify-center shadow-lg">
                  <Bell size={20} className="text-white lg:w-6 lg:h-6" />
                </div>
                <div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 lg:py-8">
          ...
          <div className="relative z-10 flex items-center justify-between">
            <div>
              ...
              <p className="text-indigo-100 text-sm lg:text-base">
                dari {jadwalList.length} total jadwal
              </p>
            </div>
        ...
        <AnimatePresence mode="wait">
          ...
          {!loading && filteredJadwalList.length === 0 && jadwalList.length > 0 && (
            <motion.div
              key="no-results"
              initial={{ opacity: 0, scale: 0.95 }}
              animate={{ opacity: 1, scale: 1 }}
          ...
          {!loading && filteredJadwalList.length > 0 && (
            <motion.div
              ...
              transition={{ duration: 0.5, delay: 0.2 }}
            >
              {/* Kontainer untuk dua tabel jadwal */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-12">
                <JadwalListSection
                  title="Jadwal Ibadah Mingguan"
                  icon={CalendarDays}
                  jadwalItems={jadwalMingguan}
                />
                <JadwalListSection
                  title="Jadwal Ibadah Gabungan"
                  icon={Users}
                  jadwalItems={jadwalGabungan}
                />
              </div>
            </motion.div>
          )}
          ...
          >
            <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-2xl p-6 border border-blue-200/50">
              <p className="text-gray-700 text-sm lg:text-base">
                💡 <span className="font-semibold">Tips:</span> Jadwal dapat berubah sewaktu-waktu. Harap konfirmasi melalui pengumuman terbaru di gereja.
              </p>
            </div>
          </motion.div>
        )}
